...
function buildNativeServiceCompanionHostSnippet() {
  const body = [
    ...
    "      hasCapacitorGlobalAddListener: Boolean(capacitor && typeof capacitor.addListener === 'function'),",
    ...
    '  };',
    "  const CAPTURE_ROUTE_PROVIDER_KIND = 'capacitor-android-capture-routes';",
    '  const captureRouteDiagnostics = { latestStatus: null, latestStart: null, latestStop: null, latestError: null, latestChunk: null, latestListener: null };',
    '  let captureRouteChunkHandler = null;',
    '  let captureRouteNativeListenerPromise = null;',
    '  const normalizeCaptureRouteKind = function(value) {',
    "    return value === 'wrapper-preferred-microphone' || value === 'wrapper-loopback' ? value : 'wrapper-preferred-microphone';",
    '  };',
    '  const recordCaptureRouteError = function(stage, error) {',
    '    captureRouteDiagnostics.latestError = {',
    '      stage: stage || null,',
    '      atMs: Date.now(),',
    ...
    '    };',
    '  };',
    '  const normalizeFloatChunkBuffer = function(rawBuffer) {',
    '    if (rawBuffer instanceof Float32Array) return rawBuffer;',
    '    if (Array.isArray(rawBuffer)) return new Float32Array(rawBuffer.map(function(value) { return Number(value) || 0; }));',
    '    return new Float32Array(0);',
    '  };',
    '  const emitCaptureRouteChunk = function(rawChunk) {',
    "    if (!rawChunk || typeof rawChunk !== 'object') return;",
    '    const chunk = {',
    "      routeKind: normalizeCaptureRouteKind(rawChunk.routeKind),",
    '      buffer: normalizeFloatChunkBuffer(rawChunk.buffer),',
    '      sampleRateHz: Number(rawChunk.sampleRateHz) || 48000,',
    '      capturedAtMs: Number.isFinite(Number(rawChunk.capturedAtMs)) ? Number(rawChunk.capturedAtMs) : Date.now(),',
    '      sequence: Number.isFinite(Number(rawChunk.sequence)) ? Number(rawChunk.sequence) : null,',
    "      diagnostics: rawChunk.diagnostics && typeof rawChunk.diagnostics === 'object' ? rawChunk.diagnostics : null,",
    '    };',
    "    captureRouteDiagnostics.latestChunk = { routeKind: chunk.routeKind, sampleCount: chunk.buffer.length, sampleRateHz: chunk.sampleRateHz, capturedAtMs: chunk.capturedAtMs, sequence: chunk.sequence, atMs: Date.now() };",
    "    if (typeof captureRouteChunkHandler === 'function') captureRouteChunkHandler(chunk);",
    '  };',
    '  window.addEventListener("axolync:capture-route-chunk", function(event) {',
    '    emitCaptureRouteChunk(event && event.detail);',
    '  });',
    '  const ensureCaptureRouteNativeListener = function() {',
    '    if (captureRouteNativeListenerPromise) return captureRouteNativeListenerPromise;',
    '    captureRouteNativeListenerPromise = Promise.resolve().then(function() {',
    '      return ensureBridgeRuntimeLoaded();',
    '    }).then(function() {',
    '      const capacitor = window.Capacitor || null;',
    "      if (!capacitor || typeof capacitor.addListener !== 'function') {",
    "        captureRouteDiagnostics.latestListener = { status: 'unavailable', path: 'capacitor-global-addListener', atMs: Date.now(), reason: 'Capacitor global listener API is unavailable.' };",
    '        return null;',
    '      }',
    "      captureRouteDiagnostics.latestListener = { status: 'registering', path: 'capacitor-global-addListener', atMs: Date.now() };",
    "      return Promise.resolve(capacitor.addListener('AxolyncNativeServiceCompanionHost', 'captureRouteChunk', emitCaptureRouteChunk)).then(function(listener) {",
    "        captureRouteDiagnostics.latestListener = { status: 'registered', path: 'capacitor-global-addListener', atMs: Date.now(), hasRemove: Boolean(listener && typeof listener.remove === 'function') };",
    '        return listener;',
    '      });',
    '    }).catch(function(error) {',
    "      recordCaptureRouteError('addListener', error);",
    "      captureRouteDiagnostics.latestListener = { status: 'failed', path: 'capacitor-global-addListener', atMs: Date.now(), message: error && typeof error.message === 'string' ? error.message : String(error) };",
    '      return null;',
    '    });',
    '    return captureRouteNativeListenerPromise;',
    '  };',
    '  const getCaptureRouteStatus = async function() {',
    '    try {',
    "      const status = await invoke('getCaptureRouteStatus', {});",
    "      captureRouteDiagnostics.latestStatus = { status, atMs: Date.now() };",
    '      return status;',
    '    } catch (error) {',
    "      recordCaptureRouteError('getStatus', error);",
    '      return {',
    '        capabilities: [',
    "          { kind: 'wrapper-preferred-microphone', support: 'unavailable', label: 'Android native microphone', reason: error && typeof error.message === 'string' ? error.message : String(error), requiresPermission: true },",
    "          { kind: 'wrapper-loopback', support: 'unsupported', label: 'Android playback capture', reason: 'Android playback capture bridge is unavailable.', requiresPermission: true },",
    '        ],',
    '        recommendedRoute: null,',
    "        recommendationReason: 'Capacitor capture route bridge is unavailable.',",
    "        providerKind: CAPTURE_ROUTE_PROVIDER_KIND,",
    '        diagnostics: { latestError: captureRouteDiagnostics.latestError },',
    '      };',
    '    }',
    '  };',
    '  const startCaptureRoute = async function(input) {',
    '    const routeKind = normalizeCaptureRouteKind(input && input.routeKind);',
    '    try {',
    '      if (captureRouteChunkHandler) await ensureCaptureRouteNativeListener();',
    "      const result = await invoke('startCaptureRoute', { routeKind });",
    "      if (result && typeof result === 'object') {",
    "        result.diagnostics = Object.assign({}, result.diagnostics || {}, { bridgeChunkListener: captureRouteDiagnostics.latestListener });",
    '      }',
    "      captureRouteDiagnostics.latestStart = { routeKind, result, atMs: Date.now() };",
    '      return result;',
    '    } catch (error) {',
    "      recordCaptureRouteError('start', error);",
    "      return { routeKind, routeId: null, sampleRateHz: null, diagnostics: { providerKind: CAPTURE_ROUTE_PROVIDER_KIND, error: error && typeof error.message === 'string' ? error.message : String(error) } };",
    '    }',
    '  };',
    '  const stopCaptureRoute = async function(input) {',
    '    try {',
    "      const result = await invoke('stopCaptureRoute', { routeKind: normalizeCaptureRouteKind(input && input.routeKind), routeId: input && input.routeId ? String(input.routeId) : null });",
    "      captureRouteDiagnostics.latestStop = { input: input || null, result, atMs: Date.now() };",
    '      return result;',
    '    } catch (error) {',
    "      recordCaptureRouteError('stop', error);",
    "      return { status: 'failed', reason: error && typeof error.message === 'string' ? error.message : String(error) };",
    '    }',
    '  };',
    '  const setCaptureRouteChunkHandler = async function(handler) {',
    "    captureRouteChunkHandler = typeof handler === 'function' ? handler : null;",
    '    if (captureRouteChunkHandler) await ensureCaptureRouteNativeListener();',
    "    return { status: 'success', providerKind: CAPTURE_ROUTE_PROVIDER_KIND, hasHandler: typeof captureRouteChunkHandler === 'function', listener: captureRouteDiagnostics.latestListener };",
    '  };',
    "  const LIVE_NOTIFICATION_PROVIDER_KIND = 'capacitor-local-notifications';",
    "  const LIVE_NOTIFICATION_DETECTED_CHANNEL_ID = 'axolync-live-song-detected';",
    "  const LIVE_NOTIFICATION_LYRICS_READY_CHANNEL_ID = 'axolync-live-song-lyrics-ready';",
    '  const liveNotificationDiagnostics = { providerKind: LIVE_NOTIFICATION_PROVIDER_KIND, latestPermission: null, latestShow: null, latestClear: null, latestError: null, channelsCreated: false };',
    '  const resolveLocalNotificationsPlugin = function() {',
    "    return window.Capacitor && window.Capacitor.Plugins ? window.Capacitor.Plugins.LocalNotifications : null;",
    '  };',
    '  const buildLiveNotificationSnapshot = function() {',
    '    const capacitor = window.Capacitor || null;',
    '    const plugin = resolveLocalNotificationsPlugin();',
    '    return {',
    '      providerKind: LIVE_NOTIFICATION_PROVIDER_KIND,',
    "      pluginName: 'LocalNotifications',",
    "      pluginPackageName: '@capacitor/local-notifications',",
    "      pluginClasspath: 'com.capacitorjs.plugins.localnotifications.LocalNotificationsPlugin',",
    '      hasWindowCapacitor: Boolean(capacitor),',
    '      hasCapacitorPluginsContainer: Boolean(capacitor && capacitor.Plugins),',
    '      hasPublishedLocalNotificationsPlugin: Boolean(plugin),',
    "      localNotificationsPluginMethodNames: plugin && typeof plugin === 'object' ? Object.keys(plugin).sort() : [],",
    "      hasCapacitorNativePromise: Boolean(capacitor && typeof capacitor.nativePromise === 'function'),",
    '      diagnostics: { latestPermission: liveNotificationDiagnostics.latestPermission, latestShow: liveNotificationDiagnostics.latestShow, latestClear: liveNotificationDiagnostics.latestClear, latestError: liveNotificationDiagnostics.latestError, channelsCreated: liveNotificationDiagnostics.channelsCreated },',
    '    };',
    '  };',
    '  const recordLiveNotificationError = function(stage, error) {',
    '    liveNotificationDiagnostics.latestError = {',
    '      stage: stage || null,',
    '      atMs: Date.now(),',
    "      message: error && typeof error.message === 'string' ? error.message : String(error),",
    "      code: error && typeof error.code === 'string' ? error.code : null,",
    '      bootstrapSnapshot: buildLiveNotificationSnapshot(),',
    '    };',
    '  };',
    '  const buildUnavailableLocalNotificationsError = function(methodName) {',
    '    const error = new Error("Capacitor LocalNotifications bridge is unavailable.");',
    "    error.code = 'axolync_capacitor_local_notifications_unavailable';",
    '    error.details = buildLiveNotificationSnapshot();',
    '    error.methodName = methodName || null;',
    '    return error;',
    '  };',
    '  const invokeLocalNotifications = async function(methodName, payload) {',
    '    let plugin = resolveLocalNotificationsPlugin();',
    "    if (plugin && typeof plugin[methodName] === 'function') return plugin[methodName](payload || {});",
    '    let nativePromise = resolveNativePromise();',
    "    if (nativePromise) return nativePromise('LocalNotifications', methodName, payload || {});",
    '    await ensureBridgeRuntimeLoaded();',
    '    plugin = resolveLocalNotificationsPlugin();',
    "    if (plugin && typeof plugin[methodName] === 'function') return plugin[methodName](payload || {});",
    '    nativePromise = resolveNativePromise();',
    "    if (nativePromise) return nativePromise('LocalNotifications', methodName, payload || {});",
    '    throw buildUnavailableLocalNotificationsError(methodName);',
    '  };',
    '  const normalizePermissionState = function(value) {',
    "    const display = value && typeof value.display === 'string' ? value.display : null;",
    "    return display === 'granted' || display === 'denied' || display === 'prompt' ? display : 'prompt';",
    '  };',
    '  const buildStableNotificationId = function(value) {',
    "    const text = String(value || 'axolync-live-song');",
    '    let hash = 2166136261;',
    ...
    '  };',
    '  const ensureLiveNotificationChannels = async function() {',
    '    if (liveNotificationDiagnostics.channelsCreated) return;',
    "    await invokeLocalNotifications('createChannel', { id: LIVE_NOTIFICATION_DETECTED_CHANNEL_ID, name: 'Detected songs', description: 'Quiet Axolync song detection notifications.', importance: 2, visibility: 1, vibration: false });",
    "    await invokeLocalNotifications('createChannel', { id: LIVE_NOTIFICATION_LYRICS_READY_CHANNEL_ID, name: 'Lyrics ready', description: 'Axolync lyric-ready notifications.', importance: 3, visibility: 1, vibration: true });",
    '    liveNotificationDiagnostics.channelsCreated = true;',
    '  };',
    '  const getLiveSongNotificationCapabilities = async function() {',
    '    return { supportsPermissionRequest: true, supportsSilent: true, supportsBuzz: true, supportsReplace: true, supportsClear: true, providerKind: LIVE_NOTIFICATION_PROVIDER_KIND, diagnostics: buildLiveNotificationSnapshot() };',
    '  };',
    '  const requestLiveSongNotificationPermission = async function(input) {',
    '    try {',
    "      const checked = await invokeLocalNotifications('checkPermissions', {});",
    '      let state = normalizePermissionState(checked);',
    '      let result = checked;',
    "      if (state !== 'granted') {",
    "        result = await invokeLocalNotifications('requestPermissions', {});",
    '        state = normalizePermissionState(result);',
    '      }',
    '      liveNotificationDiagnostics.latestPermission = { state, reason: input && typeof input.reason === "string" ? input.reason : null, raw: result, atMs: Date.now() };',
    "      return state === 'granted' ? { state } : { state, reason: 'capacitor local notifications permission ' + state };",
    '    } catch (error) {',
    "      recordLiveNotificationError('requestPermission', error);",
    "      return { state: 'unsupported', reason: error && typeof error.message === 'string' ? error.message : String(error) };",
    '    }',
    '  };',
    '  const clearLiveSongNotification = async function(input) {',
    '    const notificationId = buildStableNotificationId(input && input.id);',
    '    try {',
    "      await invokeLocalNotifications('cancel', { notifications: [{ id: notificationId }] });",
    "      try { await invokeLocalNotifications('removeDeliveredNotifications', { notifications: [{ id: notificationId }] }); } catch (removeError) { recordLiveNotificationError('removeDeliveredNotifications', removeError); }",
    '      liveNotificationDiagnostics.latestClear = { id: input && input.id ? String(input.id) : null, notificationId, status: "success", atMs: Date.now() };',
    "      return { status: 'success' };",
    '    } catch (error) {',
    "      recordLiveNotificationError('clear', error);",
    "      return { status: 'failed', reason: error && typeof error.message === 'string' ? error.message : String(error) };",
    '    }',
    '  };',
    '  const showLiveSongNotification = async function(input) {',
    '    const notificationId = buildStableNotificationId(input && input.id);',
    "    const phase = input && input.phase === 'lyrics-ready' ? 'lyrics-ready' : 'detected';",
    "    const channelId = phase === 'lyrics-ready' ? LIVE_NOTIFICATION_LYRICS_READY_CHANNEL_ID : LIVE_NOTIFICATION_DETECTED_CHANNEL_ID;",
    '    try {',
    '      await ensureLiveNotificationChannels();',
    '      await clearLiveSongNotification({ id: input && input.id });',
    "      const scheduled = await invokeLocalNotifications('schedule', { notifications: [{ id: notificationId, title: input && typeof input.title === 'string' ? input.title : 'Detected song', body: input && typeof input.body === 'string' ? input.body : '', channelId, ongoing: false, autoCancel: false, silent: true, extra: { axolyncLiveSongNotificationId: input && input.id ? String(input.id) : null, phase, providerKind: LIVE_NOTIFICATION_PROVIDER_KIND } }] });",
    '      liveNotificationDiagnostics.latestShow = { id: input && input.id ? String(input.id) : null, notificationId, phase, channelId, buzz: Boolean(input && input.buzz), silent: input ? input.silent !== false : true, raw: scheduled, atMs: Date.now() };',
    "      return { status: 'success' };",
    '    } catch (error) {',
    "      recordLiveNotificationError('show', error);",
    "      return { status: 'failed', reason: error && typeof error.message === 'string' ? error.message : String(error) };",
    '    }',
    '  };',
    '  const hostMetadata = {',
    ...
    '    },',
    '    liveSongNotificationProviderKind: LIVE_NOTIFICATION_PROVIDER_KIND,',
    '    getLiveSongNotificationCapabilities,',
    '    requestNotificationPermission: requestLiveSongNotificationPermission,',
    '    showLiveSongNotification,',
    '    clearLiveSongNotification,',
    '    captureRouteProviderKind: CAPTURE_ROUTE_PROVIDER_KIND,',
    '    getCaptureRouteStatus,',
    '    startCaptureRoute,',
    '    stopCaptureRoute,',
    '    setCaptureRouteChunkHandler,',
    '  });',
