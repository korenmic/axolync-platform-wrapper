...
function buildNativeServiceCompanionHostSnippet() {
  const body = [
    ...
    '  };',
    "  const NATIVE_MICROPHONE_PROVIDER_KIND = 'capacitor-android-native-microphone';",
    '  let nativeMicrophoneChunkHandler = null;',
    '  let nativeMicrophoneListenerHandle = null;',
    '  const nativeMicrophoneDiagnostics = { providerKind: NATIVE_MICROPHONE_PROVIDER_KIND, latestListener: null, latestStatus: null, latestStart: null, latestStop: null, latestChunk: null, latestError: null };',
    '  const recordNativeMicrophoneError = function(stage, error) {',
    '    nativeMicrophoneDiagnostics.latestError = {',
    '      stage: stage || null,',
    '      atMs: Date.now(),',
    ...
    '    };',
    '  };',
    '  const emitNativeMicrophoneChunk = function(event) {',
    '    nativeMicrophoneDiagnostics.latestChunk = { atMs: Date.now(), sequence: event && typeof event === "object" ? event.sequence : null, sampleCount: event && typeof event === "object" ? event.sampleCount : null, sampleRateHz: event && typeof event === "object" ? event.sampleRateHz : null };',
    "    if (typeof nativeMicrophoneChunkHandler === 'function') nativeMicrophoneChunkHandler(event);",
    '  };',
    '  const registerNativeMicrophoneListener = async function() {',
    '    if (nativeMicrophoneListenerHandle) {',
    "      const existing = { status: 'registered', path: 'capacitor-global-addListener', reason: 'already-registered' };",
    '      nativeMicrophoneDiagnostics.latestListener = existing;',
    '      return existing;',
    '    }',
    '    const capacitor = window.Capacitor || null;',
    "    if (!capacitor || typeof capacitor.addListener !== 'function') {",
    "      const unavailable = { status: 'unavailable', path: 'capacitor-global-addListener', reason: 'capacitor-global-addListener-unavailable' };",
    '      nativeMicrophoneDiagnostics.latestListener = unavailable;',
    '      return unavailable;',
    '    }',
    '    try {',
    "      nativeMicrophoneListenerHandle = await capacitor.addListener('AxolyncNativeServiceCompanionHost', 'nativeMicrophoneChunk', emitNativeMicrophoneChunk);",
    "      const registered = { status: 'registered', path: 'capacitor-global-addListener' };",
    '      nativeMicrophoneDiagnostics.latestListener = registered;',
    '      return registered;',
    '    } catch (error) {',
    "      const failed = { status: 'failed', path: 'capacitor-global-addListener', reason: error && typeof error.message === 'string' ? error.message : String(error) };",
    '      nativeMicrophoneDiagnostics.latestListener = failed;',
    "      recordNativeMicrophoneError('listener', error);",
    '      return failed;',
    '    }',
    '  };',
    '  const setNativeMicrophoneChunkHandler = async function(handler) {',
    "    nativeMicrophoneChunkHandler = typeof handler === 'function' ? handler : null;",
    '    if (!nativeMicrophoneChunkHandler) {',
    "      return { status: 'unavailable', path: 'capacitor-global-addListener', reason: 'handler-cleared' };",
    '    }',
    '    return registerNativeMicrophoneListener();',
    '  };',
    '  const getNativeMicrophoneRouteStatus = async function() {',
    '    try {',
    "      const status = await invoke('getNativeMicrophoneRouteStatus', {});",
    '      if (status && typeof status === "object") status.diagnostics = Object.assign({}, status.diagnostics || {}, { listener: nativeMicrophoneDiagnostics.latestListener, latestChunk: nativeMicrophoneDiagnostics.latestChunk, latestError: nativeMicrophoneDiagnostics.latestError });',
    '      nativeMicrophoneDiagnostics.latestStatus = status;',
    '      return status;',
    '    } catch (error) {',
    "      recordNativeMicrophoneError('status', error);",
    "      return { available: false, support: 'unavailable', providerKind: NATIVE_MICROPHONE_PROVIDER_KIND, reason: error && typeof error.message === 'string' ? error.message : String(error), diagnostics: nativeMicrophoneDiagnostics };",
    '    }',
    '  };',
    '  const startNativeMicrophoneRoute = async function() {',
    '    const listener = await registerNativeMicrophoneListener();',
    "    if (listener.status !== 'registered') return Object.assign({ available: false, support: 'unavailable', providerKind: NATIVE_MICROPHONE_PROVIDER_KIND }, listener);",
    "    const result = await invoke('startNativeMicrophoneRoute', {});",
    '    nativeMicrophoneDiagnostics.latestStart = { atMs: Date.now(), result };',
    '    return result;',
    '  };',
    '  const stopNativeMicrophoneRoute = async function() {',
    "    const result = await invoke('stopNativeMicrophoneRoute', {});",
    '    nativeMicrophoneDiagnostics.latestStop = { atMs: Date.now(), result };',
    '    return result;',
    '  };',
