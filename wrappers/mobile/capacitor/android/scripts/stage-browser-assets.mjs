...
function buildNativeServiceCompanionHostSnippet() {
  const body = [
    ...
    "        captureRouteDiagnostics.latestListener = { status: 'unavailable', atMs: Date.now(), reason: 'Capacitor plugin listener API is unavailable.' };",
    '        return null;',
    '      }',
    "      captureRouteDiagnostics.latestListener = { status: 'registering', atMs: Date.now() };",
    "      return Promise.resolve(plugin.addListener('captureRouteChunk', emitCaptureRouteChunk)).then(function(listener) {",
    "        captureRouteDiagnostics.latestListener = { status: 'registered', atMs: Date.now(), hasRemove: Boolean(listener && typeof listener.remove === 'function') };",
    '        return listener;',
    '      });',
    '    }).catch(function(error) {',
    "      recordCaptureRouteError('addListener', error);",
    "      captureRouteDiagnostics.latestListener = { status: 'failed', atMs: Date.now(), message: error && typeof error.message === 'string' ? error.message : String(error) };",
    '      return null;',
    '    });',
